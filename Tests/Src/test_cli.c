...
void test_vCLIparseString ( void )
{
  ...
  TEST_ASSERT_GREATER_THAN_UINT8( 0U, message.dataFlag );
  TEST_ASSERT_EQUAL( 0U,              message.data[0U] );
  vCLIparseString( "get dout 2", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET, message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_DOUT, message.target );
  TEST_ASSERT_GREATER_THAN_UINT8( 0U, message.dataFlag );
  TEST_ASSERT_EQUAL( 2U,              message.data[0U] );
  ...
  TEST_ASSERT_GREATER_THAN_UINT8( 0U,      message.dataFlag );
  TEST_ASSERT_EQUAL( 1U,                   message.data[0U] );
  ...
  TEST_ASSERT_GREATER_THAN_UINT8( 0U,    message.dataFlag );
  TEST_ASSERT_EQUAL( 3U,                 message.data[0U] );
  ...
  TEST_ASSERT_EQUAL( 0U,                 message.dataFlag );
  vCLIparseString( "get unique", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET,   message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_UNIQUE, message.target );
  TEST_ASSERT_EQUAL( 0U,                message.dataFlag );
  vCLIparseString( "get released", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET,    message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_RELEASE, message.target );
  TEST_ASSERT_EQUAL( 0U,                 message.dataFlag );
  vCLIparseString( "get serial", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET,   message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_SERIAL, message.target );
  TEST_ASSERT_EQUAL( 0U,                message.dataFlag );
  vCLIparseString( "get ip", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET, message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_IP,   message.target );
  TEST_ASSERT_EQUAL( 0U,              message.dataFlag );
  vCLIparseString( "get mac", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET, message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_MAC,  message.target );
  TEST_ASSERT_EQUAL( 0U,              message.dataFlag );
  vCLIparseString( "get version", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET,    message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_VERSION, message.target );
  TEST_ASSERT_EQUAL( 0U,                 message.dataFlag );
  vCLIparseString( "get adr", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET,       message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_MODBUS_ADR, message.target );
  TEST_ASSERT_EQUAL( 0U,                    message.dataFlag );
  vCLIparseString( "get baudrate", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET,      message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_MODBUS_BR, message.target );
  TEST_ASSERT_EQUAL( 0U,                   message.dataFlag );
  vCLIparseString( "get sd", &message );
  TEST_ASSERT_EQUAL( CLI_COMMAND_GET, message.cmd );
  TEST_ASSERT_EQUAL( CLI_TARGET_SD,   message.target );
  TEST_ASSERT_EQUAL( 0U,              message.dataFlag );
  return;
}
...
void test_uCLItimeToStr ( void )
{
  ...
  time.day   = 21U;
  time.month = 10U;
  time.year  = 2022U - LOG_START_YEAR;
  time.hour  = 11U;
  time.min   = 45U;
  time.sec   = 54U;
...
void test_uCLIparse ( void )
{
  ...
  const char* const dictionary[3U] = { "loo", "foo", "goo" };
