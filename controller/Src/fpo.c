...
void vFPOinit( const FPO_INIT* init )
{
  ...
  fpos[FPO_G].port          = init->outPortG;
  fpos[FPO_G].pin           = init->outPinG;
  fpos[FPO_H].port          = init->outPortH;
  fpos[FPO_H].pin           = init->outPinH;
  fpos_dis[FPO_DIS_GH].port = init->disPortGH;
  fpos_dis[FPO_DIS_GH].pin  = init->disPinGH;

  fpos[FPO_I].port          = init->outPortI;
  fpos[FPO_I].pin           = init->outPinI;
  fpos[FPO_J].port          = init->outPortJ;
  fpos[FPO_J].pin           = init->outPinJ;
  fpos_dis[FPO_DIS_IJ].port = init->disPortIJ;
  fpos_dis[FPO_DIS_IJ].pin  = init->disPinIJ;
  /* Data init */
  vFPOdataInit();
  return;
}
