...
static eConfigBitMap oilPressureSetupBitMap[4U] = 
{
   ...
   { 16U, 4U, 0U, 1U },     // oilPressureEnbOpenCircuitAlarm
...
static eConfigBitMap coolantTempSetupBitMap[6U] = 
{
   ...
   { 16U, 4U, 0U, 1U },     // coolantTempEnbOpenCircuitAlarm
...
static eConfigBitMap fuelLevelSetupBitMap[8U] = 
{
   ...
   { 64U, 6U, 0U, 1U },     // fuelLevelHightPreAlarmLevelEnb
...
eConfigReg genRatedActivePower =
{
   .page       = 0U,
   .adr        = 52U,
   .scale      = 0U,
...
eConfigReg genRatedReactivePower =
{
   .page       = 0U,
   .adr        = 53U,
   .scale      = 0U,
...
eConfigReg genRatedApparentPower =
{
   .page       = 0U,
   .adr        = 54U,
   .scale      = 0U,
...
static eConfigBitMap genAlarmsBitMap[12U] = 
{
   ...
   { 32U, 5U, 0U, 1U },     // genOverFrequencyPreAlrmEnb
...
eConfigReg genUnderFrequencyAlrmLevel =
{
   .page       = 0U,
   .adr        = 63U,
   .scale      = -1,
...
eConfigReg genUnderFrequencyPreAlrmLevel =
{
   .page       = 0U,
   .adr        = 64U,
   .scale      = -1,
...
eConfigReg genOverFrequencyPreAlrmLevel =
{
   .page       = 0U,
   .adr        = 65U,
   .scale      = -1,
...
eConfigReg genOverFrequencyAlrmLevel =
{
   .page       = 0U,
   .adr        = 66U,
   .scale      = -1,
...
{
   { 1U, 0U, 0U, 1U },     // mainsControl
   { 2U, 1U, 0U, 1U },     // mainsPowerOffImmediately
...
static eConfigBitMap crankSetupBitMap[4U] = 
{
   ...
   { 2U, 1U, 0U, 1U },     // crankOilPressureCheckOnStart
...
eConfigReg* configReg[SETTING_REGISTER_NUMBER] = { &moduleSetup, &oilPressureSetup, &oilPressureAlarmLevel, &oilPressurePreAlarmLevel, &coolantTempSetup, &coolantHightTempAlarmLevel, &coolantHightTempPreAlarmLevel, &coolantTempHeaterOffLevel, &coolantTempHeaterOnLevel, &coolantTempCoolerOffLevel, &coolantTempCoolerOnLevel, &fuelLevelSetup, &fuelLevelLowAlarmLevel, &fuelLevelLowAlarmDelay, &fuelLevelLowPreAlarmLevel, &fuelLevelLowPreAlarmDelay, &fuelLevelHightPreAlarmLevel, &fuelLevelHightPreAlarmDelay, &fuelLevelHightAlarmLevel, &fuelLevelHightAlarmDelay, &fuelPumpOnLevel, &fuelPumpOffLevel, &diaSetup, &diaDelay, &dibSetup, &dibDelay, &dicSetup, &dicDelay, &didSetup, &didDelay, &doSetup, &doabType, &dodType, &doefType, &timerMainsTransientDelay, &timerStartDelay, &timerCranking, &timerCrankDelay, &timerStartupIdleTime, &timerNominalRPMDelay, &timerSafetyOnDelay, &timerWarming, &timerTransferDelay, &timerBreakerTripPulse, &timerBreakerClosePulse, &timerReturnDelay, &timerCooling, &timerCoolingIdle, &timerSolenoidHold, &timerFailStopDelay, &timerGenTransientDelay, &genSetup, &genRatedActivePower, &genRatedReactivePower, &genRatedApparentPower, &genRatedFrequency, &genCurrentPrimary, &genCurrentFullLoadRating, &genAlarms, &genUnderVoltageAlarmLevel, &genUnderVoltagePreAlarmLevel, &genOverVoltagePreAlarmLevel, &genOverVoltageAlarmLevel, &genUnderFrequencyAlrmLevel, &genUnderFrequencyPreAlrmLevel, &genOverFrequencyPreAlrmLevel, &genOverFrequencyAlrmLevel, &genOverCurrentThermalProtectionLevel, &genOverCurrentCutoffLevel, &genOverCurrentAlarmLevel, &genOverCurrentAlarmDelay, &genCurrentOverloadProtectionLevel, &genCurrentOverloadProtectionDelay, &genCurrentOverPhaseImbalanceLevel, &genCurrentOverPhaseImbalanceDelay, &mainsSetup, &mainsAlarms, &mainsUnderVoltageAlarmLevel, &mainsOverVoltageAlarmLevel, &mainsUnderFrequencyAlarmLevel, &mainsOverFrequencyAlarmLevel, &engineSetup, &enginePreHeatOn, &enginePreHeatDuration, &enginePostHeatOn, &enginePostHeatDuration, &crankSetup, &crankDisconnectgenFreqLevel, &crankDisconnectOilPressureLevel, &crankDisconnectChargeAlternatorLevel, &batteryAlarms, &batteryUnderVoltageLevel, &batteryUnderVoltageDelay, &batteryOverVoltageLevel, &batteryOverVoltageDelay, &batteryChargeShutdownLevel, &batteryChargeShutdownDelay, &batteryChargeWarningLevel, &batteryChargeWarningDelay, &maintenanceAlarms, &maintenanceAlarmOilEngineRunTime, &maintenanceAlarmAirEngineRunTime, &maintenanceAlarmFuelEngineRunTime};
