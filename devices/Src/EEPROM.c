...
/*-------------------------------- Variables ---------------------------------*/
/*-------------------------------- Functions ---------------------------------*/
EEPROM_STATUS eEEPROMwrite( uint8_t cmd, uint8_t* adr, uint8_t* data, uint8_t size );  /* Send data via SPI to the EEPROM */
EEPROM_STATUS eEEPROMread( uint8_t cmd, uint8_t* adr, uint8_t* data, uint8_t size );   /* Get data via SPI from EEPROM */
 ...
 */
EEPROM_STATUS eEEPROMwrite( uint8_t cmd, uint8_t* adr, uint8_t* data, uint8_t size )
{
 ...
 */
EEPROM_STATUS eEEPROMread( uint8_t cmd, uint8_t* adr, uint8_t* data, uint8_t size )
{
  HAL_StatusTypeDef hal        = HAL_OK;
  EEPROM_STATUS     res        = EEPROM_OK;
  uint8_t           buffer[4U] = { cmd, 0x00U, 0x00U, 0x00U };
...
EEPROM_STATUS vEEPROMpoolUntil ( EEPROM_SR_STATE target )
{
  ...
  do
  {
    ...
    osDelay( 5U );
  } while ( ( state != target ) || ( res != EEPROM_ERROR ));
  return res;
}
/*----------------------------------------------------------------------------*/
/*----------------------- PABLICK --------------------------------------------*/
 ...
 * Output: Status of operation
 */
EEPROM_STATUS eEEPROMReadMemory ( uint8_t* adr, uint8_t* data, uint32_t len )
{
  EEPROM_STATUS res        = EEPROM_OK;
