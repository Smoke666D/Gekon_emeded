...
static   float              MIN_PRESENT_FREQ                             = 20.0f;


uint8_t  vADCGetADC3Data();
uint8_t  vADCGetADC12Data();
void     vADCInit(void);
uint8_t  vADCFindFreq(int16_t * data, uint16_t * count,uint8_t off, int16_t AMP);// __attribute__((optimize("-O3")));
fix16_t  xADCRMS(int16_t * source, uint8_t off, uint16_t size, uint8_t cc  );
int16_t  xADCMax( int16_t * source, uint8_t off, uint16_t size , uint16_t * delay, uint8_t cc );// __attribute__((optimize("-O3")));
...
void     vDecNetural(int16_t * data);// __attribute__((optimize("-O3")));
...
void vADCGeneratorDataUpdate ( void )
{
  ...
  //  temp = fix16_mul(xLCurCoof,xTransCoof);
    ...
    {
	GENERATOR_DATA[GEN_L1_CUR]  = fix16_mul(GENERATOR_DATA[GEN_L1_CUR], xTransCoof);
    ...
    switch ( xNetWiring )
    {
      ...
      case ELECTRO_SCHEME_TRIANGLE:
        ...
        {
            GENERATOR_DATA[GEN_L2_CUR]  = fix16_mul(GENERATOR_DATA[GEN_L2_CUR], xTransCoof);
        ...
        else
        {
          ...
          GENERATOR_DATA[GEN_L3_CUR]  = fix16_mul(GENERATOR_DATA[GEN_L3_CUR], xTransCoof);
...
void StartADCTask(void *argument)
{
  ...
  for(;;)
  {
    ...
    ADCDATA[4U] = ( ADC3_IN_Buffer[8U] + ADC3_IN_Buffer[17U] + ADC3_IN_Buffer[26U] + ADC3_IN_Buffer[35U] ) >> 2U;
    if ( ADC3_IN_Buffer[5U] < 1000U )
      DCCount++;
    vADCConvertToVDD( 0U );
    //Запускаем новоей преобразование
    StartADCDMA( &hadc3, ( uint32_t* )&ADC3_IN_Buffer, ( DC_SIZE * 9U ) );
    StartADCDMA( &hadc1, ( uint32_t* )&ADC1_IN_Buffer, ( DC_SIZE * 4U ) );
    __HAL_TIM_ENABLE( &htim3 );
     ...
     switch ( vADCGetADC3Data() )
     {
       case HIGH_FREQ:
         if ( OF > 0U )
         {
           ...
           if ( ADC3Freq < 2000U )  ADC3Freq = 2000U;
           vADC3FrInit( ADC3Freq );
           OF = 0U;
         }
         OF++;
       ...
       case LOW_FREQ:
         if ( OF > 0U )
         {
           ADC3Freq = ADC3Freq + ADC3Freq / 4U;
           if ( ADC3Freq > 70000U )  ADC3Freq = 40000U;
           vADC3FrInit( ADC3Freq );
           OF = 0U;
         }
         OF++;
    ...
    switch ( vADCGetADC12Data() )
    {
      ...
      case HIGH_FREQ:
        if ( OF1 > 0U )
        {
          ...
          if ( ADC2Freq < 2000U )  ADC2Freq = 2000U;
          vADC12FrInit( ADC2Freq );
          OF1 = 0U;
        }
        OF1++;
      ...
      case LOW_FREQ:
        if ( OF1 > 0U )
        {
           ...
           if ( ADC2Freq > 70000U )  ADC2Freq = 40000U;
           vADC12FrInit( ADC2Freq );
           OF1 = 0U;
         }
         OF1++;
...
uint8_t vADCGetADC12Data()
{
  ...
  // Расчет значения тока и косинуса фи
   ...
   if (result==ADC_OK)     //Если мы определили значение частоты напряжения генератора, то считаем ток через TrueRMS
   {
     ...
     xGEN_F1_CUR =   xADCRMS((int16_t *)&ADC1_IN_Buffer, 2, uCurPeriod,3 );
     xGEN_F2_CUR =   xADCRMS((int16_t *)&ADC1_IN_Buffer, 1, uCurPeriod,3 );
     xGEN_F3_CUR =   xADCRMS((int16_t *)&ADC1_IN_Buffer, 0, uCurPeriod,3 );
     /*
      * расчет косинуса фи.
      */
   ...
   else
   {
     ...
     xGEN_F1_CUR = fix16_div( fix16_from_int( xADCMax( (int16_t *)&ADC1_IN_Buffer, 2, uCurPeriod, &DF3,3 )), fix_temp );
     xGEN_F2_CUR = fix16_div( fix16_from_int( xADCMax( (int16_t *)&ADC1_IN_Buffer, 1, uCurPeriod, &DF3,3 )), fix_temp );
     xGEN_F3_CUR = fix16_div( fix16_from_int( xADCMax( (int16_t *)&ADC1_IN_Buffer, 0, uCurPeriod, &DF3,3 )), fix_temp );
   }
