...
/*----------------------------------------------------------------------------*/
void vALARMerrorToOutput ( SYSTEM_EVENT_TYPE event, uint8_t* adr, uint8_t* bit )
{
  switch ( event )
  {
...
void vALARMsetError ( SYSTEM_EVENT_TYPE event, ERROR_LIST_CMD cmd )
{
  ...
  {
    outputDataReg[ERROR0_ADR]->value[0U] = 0U;
    outputDataReg[ERROR1_ADR]->value[0U] = 0U;
  }
  else
  {
    vALARMerrorToOutput( event, &adr, &bit );
    if ( adr > 0U )
    {
      switch ( cmd )
      {
...
/*----------------------------------------------------------------------------*/
void vALARMwarningToOutput ( SYSTEM_EVENT_TYPE event, uint8_t* adr, uint8_t* bit )
{
  switch ( event )
  {
    case EVENT_OIL_LOW_PRESSURE:
...
void vALARMsetWarning ( SYSTEM_EVENT_TYPE event, ERROR_LIST_CMD cmd )
{
  ...
  if ( cmd == ERROR_LIST_CMD_ERASE )
  {
    outputDataReg[WARNING0_ADR]->value[0U] = 0U;
    outputDataReg[WARNING1_ADR]->value[0U] = 0U;
  } else {
    vALARMwarningToOutput( event, &adr, &bit );
    if ( adr > 0U )
    {
      switch ( cmd )
      {
